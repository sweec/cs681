...
public class ThreadSafeBankAccountSolution3
{
	private double balance = 0;
	private ReentrantLock lock = new ReentrantLock();;
	private Condition sufficientFundsCondition, belowUpperLimitFundsCondition;
	private ThreadSafeBankAccountSolution3 account;
	private int acctNum = AccountNumber.generate();
	
	public ThreadSafeBankAccountSolution3()
	{
	...
	public void transfer(ThreadSafeBankAccountSolution3 destination, double amount){
		if (acctNum < destination.acctNum) {
			lock.lock();
			...
			lock.unlock();
		} else if (acctNum > destination.acctNum) {
			destination.getLock().lock();
			lock.lock();
			if( this.balance < amount )
				System.out.println("Source account balance is not enough for withdraw amount");
	...
	{
		ThreadSafeBankAccountSolution3 bankAccount = new ThreadSafeBankAccountSolution3();
		ThreadSafeBankAccountSolution3 dest = new ThreadSafeBankAccountSolution3();
	...
	private class TransferRunnable implements Runnable {
		ThreadSafeBankAccountSolution3 destination;
		public TransferRunnable(ThreadSafeBankAccountSolution3 dest) {
